...
# /ffmpeg-master-latest-win64-gpl/bin/ffmpeg -f avfoundation -framerate 60 -r 18  -video_size 1920x1080 -i "4:none" -c:v libx264 -profile:v baseline -pix_fmt yuv420p -f rtsp rtsp://localhost:8554/test
# ./ffmpeg-master-latest-win64-gpl/bin/ffmpeg -f dshow -i video="screen-capture-recorder" -framerate 60 -r 18  -video_size 1920x1080 -i "4:none" -c:v libx264 -profile:v baseline -pix_fmt yuv420p -f rtsp rtsp://localhost:8554/test
# ffmpeg -f gdigrab -framerate 30 -i desktop output.mkv
./ffmpeg-master-latest-win64-gpl/bin/ffmpeg -f gdigrab -framerate 30 -i desktop -framerate 60 -r 18  -video_size 1920x1080 -c:v libx264 -profile:v baseline -pix_fmt yuv420p -f rtsp rtsp://localhost:8554/test